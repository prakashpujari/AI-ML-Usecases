...
def log_dataset_info(df: pd.DataFrame, dataset_type: str = "train"):
    ...
    mlflow.log_param(f"{dataset_type}_rows", len(df))
    mlflow.log_param(f"{dataset_type}_columns", len(df.columns))
    mlflow.log_param(f"{dataset_type}_memory_mb", df.memory_usage(deep=True).sum() / 1024**2)
    
    # Log missing values
    missing_info = df.isnull().sum().to_dict()
    mlflow.log_dict(missing_info, f"{dataset_type}_missing_values.json")
    
    # Log basic statistics
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    if len(numeric_cols) > 0:
        stats = df[numeric_cols].describe().to_dict()
        mlflow.log_dict(stats, f"{dataset_type}_statistics.json")
...
def main():
    ...
    with mlflow.start_run(run_name=f"train_{datetime.now().strftime('%Y%m%d_%H%M%S')}") as run:
        # Log run metadata
        mlflow.set_tag("model_type", "regression" if is_regression else "classification")
        ...
        mlflow.log_param("n_input_features", len(raw_feature_names))
        ...
        mlflow.log_dict({"input_features": raw_feature_names}, "input_feature_names.json")
        ...
        if is_regression:
            ...
            mlflow.log_metric("overfit_score", float(overfit_score))
        ...
        try:
            ...
            mlflow.log_artifact(artifact_path, "production_model")
